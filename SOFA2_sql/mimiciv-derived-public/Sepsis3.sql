...
DROP TABLE IF EXISTS `mimic-hr.derived.sepsis3`;
CREATE TABLE `mimic-hr.derived.sepsis3` AS 
WITH sofa AS (
    SELECT stay_id
        , starttime, endtime
...
, s1 AS (
    ...
    FROM `mimic-hr.derived.suspicion_of_infection` AS soi
    INNER JOIN sofa
