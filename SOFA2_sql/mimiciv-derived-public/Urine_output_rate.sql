...
DROP TABLE IF EXISTS `mimic-hr.derived.urine_output_rate`;
CREATE TABLE `mimic-hr.derived.urine_output_rate` AS 
WITH tm AS (
    SELECT ie.stay_id
...
, uo_tm AS (
    SELECT tm.stay_id
        ...
        , uo.urineoutput
    FROM tm
    INNER JOIN `mimic-hr.derived.urine_output` uo
...
FROM ur_stg ur
LEFT JOIN `mimic-hr.derived.weight_durations` wd
