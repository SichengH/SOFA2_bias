...
DROP TABLE IF EXISTS `mimic-hr.derived.vasoactive_agent`;
CREATE TABLE `mimic-hr.derived.vasoactive_agent` AS 
WITH tm AS (
    SELECT
        stay_id, starttime AS vasotime
    FROM `mimic-hr.derived.dobutamine`
    UNION DISTINCT
    SELECT
        stay_id, starttime AS vasotime
    FROM `mimic-hr.derived.dopamine`
    UNION DISTINCT
    SELECT
        stay_id, starttime AS vasotime
    FROM `mimic-hr.derived.epinephrine`
    UNION DISTINCT
    SELECT
        stay_id, starttime AS vasotime
    FROM `mimic-hr.derived.norepinephrine`
    UNION DISTINCT
    SELECT
        stay_id, starttime AS vasotime
    FROM `mimic-hr.derived.phenylephrine`
    UNION DISTINCT
    SELECT
        stay_id, starttime AS vasotime
    FROM `mimic-hr.derived.vasopressin`
    UNION DISTINCT
    SELECT
        stay_id, starttime AS vasotime
    FROM `mimic-hr.derived.milrinone`
    UNION DISTINCT
