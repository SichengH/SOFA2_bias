...
DROP TABLE IF EXISTS `mimic-hr.derived.ventilation`;
CREATE TABLE `mimic-hr.derived.ventilation` AS 
WITH tm AS (
    SELECT stay_id, charttime
    FROM `mimic-hr.derived.ventilator_setting`
    UNION DISTINCT
    SELECT stay_id, charttime
    FROM `mimic-hr.derived.oxygen_delivery`
)
...
, vs AS (
    ...
    FROM tm
    LEFT JOIN `mimic-hr.derived.ventilator_setting` vs
        ON tm.stay_id = vs.stay_id
            AND tm.charttime = vs.charttime
    LEFT JOIN `mimic-hr.derived.oxygen_delivery` od
