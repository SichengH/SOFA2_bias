...
DROP TABLE IF EXISTS `mimic-hr.derived.suspicion_of_infection`;
CREATE TABLE `mimic-hr.derived.suspicion_of_infection` AS 
WITH ab_tbl AS (
    SELECT
    ...
    FROM `mimic-hr.derived.antibiotic` abx
)
