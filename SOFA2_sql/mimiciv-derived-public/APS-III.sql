...
DROP TABLE IF EXISTS `mimic-hr.derived.apsiii`;
CREATE TABLE `mimic-hr.derived.apsiii` AS
WITH pa AS (
    SELECT ie.stay_id, bg.charttime
        , po2 AS pao2
        , ROW_NUMBER() OVER (PARTITION BY ie.stay_id ORDER BY bg.po2 DESC) AS rn
    FROM `mimic-hr.derived.bg` bg
...
, aa AS (
    ...
    INNER JOIN `physionet-data.mimiciv_3_1_icu.icustays` ie
        ON bg.hadm_id = ie.hadm_id
            AND bg.charttime >= ie.intime AND bg.charttime < ie.outtime
    INNER JOIN `mimic-hr.derived.ventilation` vd
        ON ie.stay_id = vd.stay_id
            AND bg.charttime >= vd.starttime
            AND bg.charttime <= vd.endtime
            AND vd.ventilation_status = 'InvasiveVent'
...
, acidbase AS (
    ...
    FROM `mimic-hr.derived.bg` bg
    INNER JOIN `lcp-consortium.mimiciv_icu.icustays` ie
        ON bg.hadm_id = ie.hadm_id
            AND bg.charttime >= ie.intime AND bg.charttime < ie.outtime
...
, arf AS (
    ...
    LEFT JOIN `mimic-hr.derived.first_day_urine_output` uo
        ON ie.stay_id = uo.stay_id
    LEFT JOIN `mimic-hr.derived.first_day_lab` labs
...
, vent AS (
    ...
    FROM `physionet-data.mimiciv_3_1_icu.icustays` ie
    LEFT JOIN `mimic-hr.derived.ventilation` v
...
, cohort AS (
    ...
    LEFT JOIN `mimic-hr.derived.first_day_gcs` gcs
        ON ie.stay_id = gcs.stay_id
    LEFT JOIN `mimic-hr.derived.first_day_vitalsign` vital
